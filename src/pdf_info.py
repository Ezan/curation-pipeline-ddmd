...
def pdf_info(html_file_path, pdf, chrome_path):
...
# obtain text layout
    ...
    point_left = sorted([(i, left_point.count(i)) for i in set(left_point)],
                        key=lambda x: x[1],
                        reverse=True)
    width_row = sorted([(i, row_width.count(i)) for i in set(row_width)],
                       key=lambda x: x[1],
                       reverse=True)
    height_row = sorted([(i, row_height.count(i)) for i in set(row_height)],
                        key=lambda x: x[1],
                        reverse=True)
    ...
    while i < len(point_left):
        ...
        while j < len(point_left):
            if abs(point_left[i][0] - point_left[j][0]) <= 10:
                point_left[i] = (point_left[i][0],
                                 point_left[i][1] + point_left[j][1])
                del point_left[j]
            else:
                j = j + 1
        i = i + 1
    ...
    if float(point_left[0][1]) / len(left_point) > 0.75 \
        ...
        # int(float((info['page_width'] - 2*point_left[0][0]))/info['row_width'])
        column_no = 2
    ...
    if left_bar > 0 and left_bar < 20 * info['row_height']:
        ...
        info['right_bbox'] = [
            min(info['page_width'] - 2 * info['row_height'],
                right_bar), 0, info['page_width'] -
            min(info['page_width'] - 2 * info['row_height'], right_bar),
            info['page_height']
        ]
        if info['text_layout'][0] < 15 * info['row_height'] and info[
                'text_layout'][1] > 15 * info['row_height']:
            info['top_bbox'] = [
                0, 0, info['page_width'], info['text_layout'][0]
            ]
            info['down_bbox'] = [
                0, info['text_layout'][1], info['page_width'],
                info['page_height'] - info['text_layout'][1]
            ]
        else:
            info['top_bbox'] = [0, 0, info['page_width'], info['row_height']]
            info['down_bbox'] = [
                0, info['page_height'] - info['row_height'],
                info['page_width'], info['row_height']
            ]
    else:
        info['left_bbox'] = [0, 0, info['row_height'], info['page_height']]
        info['right_bbox'] = [
            info['page_width'] - info['row_height'], 0, info['row_height'],
            info['page_height']
        ]
        info['top_bbox'] = [0, 0, info['page_width'], info['row_height']]
        info['down_bbox'] = [
            0, info['page_height'] - info['row_height'], info['page_width'],
            info['row_height']
        ]
